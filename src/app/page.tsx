...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('aot_final_fixed').on('postgres_changes', { event: '*', schema: 'public', table: 'trading_accounts' }, () => fetchAccounts()).subscribe()
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-200 p-2 md:p-6 font-sans uppercase tracking-tight">
      <div className="max-w-[1800px] mx-auto">
        ...
        {/* HEADER - แก้ไข Layout ให้ปุ่มกลับมา */}
        <div className="flex flex-col md:flex-row items-center justify-between mb-6 bg-slate-900/40 p-4 md:px-8 rounded-[2rem] md:rounded-full border border-slate-800 backdrop-blur-xl shadow-2xl gap-4">
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 w-full md:w-auto">
            ...
            {/* 🔥 ปุ่มสลับ VIEW + ปุ่มเลือก COLS (จุดที่แก้) */}
                ...
                {sortedAccounts.map((acc) => {
                  ...
                  return (
                    <tr key={acc.account_id} className={`hover:bg-slate-800/30 transition-colors ${acc.is_demo ? 'opacity-60' : ''}`}>
                      ...
                      <td className="p-4 text-right font-mono font-bold"><div className="text-white">{acc.equity.toLocaleString(undefined, {minimumFractionDigits: 1})} <span className="text-[8px] text-slate-500">{acc.is_usc ? 'USC' : 'USD'}</span></div><div className="text-[9px] text-slate-500 opacity-60">{acc.balance.toLocaleString()}</div></td>
              ...
              return (
                <div key={acc.account_id} className={`bg-slate-900 border-2 rounded-[2rem] p-6 shadow-2xl transition-all duration-500 ${offline ? 'border-red-500/40' : 'border-slate-800 hover:border-blue-500/50'} ${acc.is_demo ? 'opacity-80' : ''}`}>
                  <div className="flex justify-between items-start mb-6 uppercase">
                    <div><h2 className={`text-base font-black truncate max-w-[150px] ${offline ? 'text-red-400' : 'text-white'}`}>{acc.account_name}</h2><p className="text-[9px] text-slate-500 font-mono">ID: {acc.account_id}</p></div>
