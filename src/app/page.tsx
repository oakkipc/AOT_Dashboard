...
export default function Home() {
  ...
  const isStale = (updatedAt: string) => {
    ...
    // เช็ค Offline โดยเผื่อส่วนต่าง Timezone
    const effectiveDiff = diffInSeconds > 20000 ? Math.abs(diffInSeconds - 25200) : diffInSeconds
    return effectiveDiff > 180 
  }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('aot_final_v15').on('postgres_changes', { event: '*', schema: 'public', table: 'trading_accounts' }, () => fetchAccounts()).subscribe()
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-200 p-4 md:p-6 font-sans uppercase tracking-tight">
      <div className="max-w-[1800px] mx-auto">
        ...
        {/* HEADER - ระนาบบรรทัดเดียวพร้อม Filter */}
        <div className="flex flex-row items-center justify-between mb-8 bg-slate-900/40 p-4 px-8 rounded-full border border-slate-800 backdrop-blur-xl shadow-2xl overflow-x-auto no-scrollbar">
          ...
          <div className="flex items-center gap-6 shrink-0">
            ...
            {/* Column Selector (เฉพาะโหมด Card) */}
            {viewMode === 'grid' && (
          ...
          <div className="flex items-center gap-6 shrink-0 text-right">
            ...
            <div className="flex flex-col items-center">
               ...
               <p className="text-[8px] text-slate-500 font-bold uppercase">Live</p>
            </div>
          </div>
        </div>
        ...
        {viewMode === 'grid' && (
          <div className={`grid gap-6 grid-cols-1 ${cols === 2 ? 'md:grid-cols-2' : cols === 3 ? 'md:grid-cols-2 lg:grid-cols-3' : cols === 4 ? 'md:grid-cols-2 lg:grid-cols-4' : ''}`}>
            {accounts.map((acc) => {
              ...
              return (
                <div key={acc.account_id} className={`relative transition-all duration-500 bg-slate-900 border-2 rounded-[2.5rem] p-8 shadow-2xl ${offline ? 'border-red-500/40 grayscale-[0.2]' : 'border-slate-800 hover:border-blue-500/50'} ${acc.is_demo ? 'opacity-80 shadow-none' : ''}`}>
                  {acc.is_demo && <div className="absolute top-0 left-1/2 -translate-x-1/2 bg-amber-500 text-black text-[9px] font-black px-4 py-1 rounded-b-xl uppercase shadow-lg">Demo</div>}
                  
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h2 className={`text-xl font-black truncate ${offline ? 'text-red-400' : 'text-white'}`}>{acc.account_name}</h2>
                      <div className="flex items-center gap-2 mt-1">
                        <p className="text-slate-500 text-[10px] font-mono uppercase tracking-tighter">ID: {acc.account_id} | {acc.is_usc ? 'USC' : 'USD'}</p>
                      </div>
                    </div>
                  ...
                  <div className="text-center mb-10">
                    <p className="text-blue-400 text-[10px] font-black mb-2 tracking-widest uppercase">Current Equity</p>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6 uppercase">
                    ...
                    <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50 text-center text-slate-300">
                      <p className="text-[9px] text-slate-500 font-black mb-1">Balance</p>
                  ...
                  <div className="flex justify-between text-[10px] font-bold text-slate-600 border-t border-slate-800/50 pt-5">
                    ...
                    <span className="font-mono text-slate-400 tracking-tighter">
                      SYNC: {formatThaiTime(acc.updated_at)}
                    </span>
                  </div>
                </div>
              )
        ...
        {/* VIEW: TABLE */}
        {viewMode === 'table' && (
          <div className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-2xl uppercase">
                ...
                {accounts.map((acc) => {
                  ...
                  return (
                    <tr key={acc.account_id} className={`hover:bg-slate-800/30 transition-colors ${acc.is_demo ? 'bg-slate-900/40' : ''}`}>
                      <td className="p-6">
                        <p className="font-bold text-white text-base leading-none">{acc.account_name}</p>
                        <p className="text-[10px] text-slate-500 font-mono tracking-widest mt-1">ID: {acc.account_id}</p>
                      </td>
                      <td className="p-6 text-center font-bold text-[10px]">
                        <span className={acc.is_usc ? 'text-amber-500' : 'text-blue-400'}>{acc.is_usc ? 'USC' : 'USD'} {acc.is_demo && '(DEMO)'}</span>
                      </td>
                      ...
                      <td className="p-6 text-right font-mono font-bold text-blue-400 text-lg">
                        {acc.is_demo ? "--" : `$${equityUSD.toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
                      </td>
                      <td className={`p-6 text-center font-mono font-bold text-base ${ddPct < 0 ? 'text-red-500' : 'text-emerald-400'}`}>{ddPct.toFixed(2)}%</td>
                      <td className="p-6 text-right font-mono font-bold text-slate-400 text-sm">{formatThaiTime(acc.updated_at)}</td>
                    </tr>
                  )
                })}
              </tbody>
